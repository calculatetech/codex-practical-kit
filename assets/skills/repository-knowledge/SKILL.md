...
When `get_answer` reports `no-llm-provider`, use its complete candidate set to select a symbol or path. Prefer an exact symbol or path over a generic page. After `no-llm-provider`, do not call `get_answer` again in the same Codex session. A new session can use a provider that the user configured later.
